...
- Added settings-page CLI path visibility: `/api/settings/detect-engines` now returns configured/environment/auto-detected Claude and Codex paths, and the settings page fills empty path fields with detected values plus a source hint. Browser verification showed the actual nvm Claude/Codex paths visible in the input fields. Verified with `npm --prefix web run build`, `npm test -- server/test/routes/settings.test.js`, and full `npm test`.
- Added the desktop "启动后最小化到托盘" setting. The setting defaults off, saves as `startMinimizedToTray`, is exposed through a non-sensitive desktop-only settings endpoint, and the Electron window stays hidden on ready when enabled. Verified with `node -c`, `npm --prefix web run build`, `npm test -- server/test/routes/settings.test.js`, full `npm test`, `npm run desktop:pack`, and a packaged app smoke test returning `{"startMinimizedToTray":"true"}` from the isolated desktop data directory.
- Added a backend fallback so `GET /api/settings` fills empty `claudePath` and `codexPath` fields with detected paths. This makes the paths visible even in the older settings UI that only reads `/api/settings` and does not call `/api/settings/detect-engines`. Verified the API returns the local nvm Claude/Codex paths, plus `npm test -- server/test/routes/settings.test.js` and full `npm test`.
- Fixed task editor "测试执行" failures. The route now runs an unsaved task as a non-persisted preview instead of creating a `runs` row with a non-existent temporary task id, which previously triggered a SQLite foreign-key error and surfaced as `Internal Server Error`. Verified with a route regression test, a real `/api/tasks/test-run` smoke call returning `status=succeeded`, `node -c`, `npm --prefix web run build`, and full `npm test`.
- Reduced desktop test-run permission footprint after macOS privacy prompts appeared for Photos/Downloads. Preview runs no longer pass Claude `--permission-mode bypassPermissions`, and they execute from the app-owned `DATA_DIR/preview-runs` directory instead of inheriting a broad working directory. Verified the regression test asserts no `bypassPermissions` argument and a `preview-runs` cwd, full tests pass, the desktop package builds, and packaged `/api/tasks/test-run` returns HTTP 200 with `status=succeeded`.
- Added immediate toast feedback for task editor "测试执行" without changing the button label. The button stays as-is; clicking now shows "测试执行已开始", then completion/failure/timeout feedback. Verified with `npm --prefix web run build`.
- Clarified task editor test-run notification behavior. Test runs are preview executions and do not trigger Feishu delivery; the completion toast and result panel now say this explicitly. Verified with `npm --prefix web run build`.
- Changed task editor test-run behavior to also send Feishu notifications using the current task/settings notification targets while still avoiding persisted run history. The API returns a `notification` result so the UI can show sent/skipped/failed feedback. Verified with route tests for sent and skipped notification paths, `npm --prefix web run build`, and full `npm test`.
- Hardened test-run Feishu notifications against local information leakage. Preview executions now wrap user content in a fixed test prompt that forbids path/environment disclosure, and Feishu delivery uses a sanitized test summary instead of raw model output. Verified route tests assert no `/Users/` or `preview-runs` leaks in sent notification text, full tests pass, and the desktop package builds and starts successfully.

### 2026-06-13

- Implemented real task-chain execution semantics on `codex/task-chain-execution`. A task can now choose a parent task and a trigger mode (`仅按定时`, `父任务成功后`, or `定时 + 父任务`); parent success triggers enabled child tasks as `chain` runs, while `chain_only` tasks are no longer registered with cron. Child prompts can explicitly consume upstream output via `{{parent_summary}}`, `{{parent_result}}`, or `{{prev_output}}`. Verified with service tests, route tests, full `npm test` (16 files, 102 tests), and `npm --prefix web run build`.
